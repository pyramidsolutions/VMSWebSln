...
namespace MVSThemeTemplateWeb
{
    public partial class AddVehicleTransaction : System.Web.UI.Page
    {
        ...
        public void NextStep()
        {
            ...
            View current = this.multiViewAddTransaction.GetActiveView();

            switch (current.ID)
	        {
                case "viewAddVehicleTransactions":
                {
                    this.multiViewAddTransaction.SetActiveView(this.viewCustomerInformation);
                    
                    break;
                        
                }
                case "viewCustomerInformation":
                {
                    this.multiViewAddTransaction.SetActiveView(this.viewCreateInvoice);

                    break;
                }

                case "viewCreateInvoice":
                {
                    this.multiViewAddTransaction.SetActiveView(this.viewPrintInvoice);

                    break;
                }

		        default:
                        this.multiViewAddTransaction.SetActiveView(this.viewAddVehicleTransactions);
                        break;
        ...
        {
           int currentViewIndex = this.multiViewAddTransaction.ActiveViewIndex;
           if (currentViewIndex > 0)
	       {
               multiViewAddTransaction.ActiveViewIndex = currentViewIndex - 1;
	       }   
        ...
        protected void invoiceLoad(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Session["TRANSACTION_VEHICLES"] != null)
                {
                    ...
                    {
                        grdInvoiceVehicles.DataSource = vehicles;
                        grdInvoiceVehicles.DataBind();
                    }
                }
        ...
        protected void btnSubmitInvoice_ServerClick(object sender, EventArgs e)
        {
            NextStep();
        }
        #endregion 
        ...
        private void validateCurrentView(StringBuilder buffer)
        {

        }

        protected void multiViewAddTransaction_Load(object sender, EventArgs e)
        {

        }

        protected void btnPrevTab_ServerClick(object sender, EventArgs e)
        {
            this.PreviousStep();
            setControls();
        }
        ...
        private bool saveInvoice()
        {
            try
            {
                ManageInvoices mangageInvoices = new ManageInvoices();
                InvoiceObject invoiceObject = new InvoiceObject();
                invoiceObject.CompanyInfo = new CompanyObject();
                
                bool result = mangageInvoices.insert(
            }
            catch (Exception ex)
            {
               throw;
